...
const sendWelcomeEmail = async ({ nombreCompleto, correo, password }) => {
  ...
  `;
  await sendEmail({ to: correo, subject: "Bienvenido a Unistock — Tu cuenta ha sido creada", html });
};
...
const sendForgotPasswordEmail = async ({ nombreCompleto, correo, codigo }) => {
  ...
  `;
  await sendEmail({ to: correo, subject: "Código de recuperación de contraseña — Unistock", html });
};
...
const sendAlertEmail = async ({ nombreCompleto, correo }) => {
  ...
  `;
  await sendEmail({ to: correo, subject: "⚠️ Alerta de seguridad — Unistock", html });
};
...
const sendPasswordChangedEmail = async ({ nombreCompleto, correo }) => {
  ...
  `;
  await sendEmail({ to: correo, subject: "Tu contraseña fue actualizada — Unistock", html });
};
...
const sendEmailChangedEmail = async ({ nombreCompleto, correoNuevo, correoAnterior }) => {
  ...
  `;
  await sendEmail({ to: correoNuevo, subject: "Tu correo electrónico fue actualizado — Unistock", html });
};
...
const sendProductionAssignedEmail = async ({ nombreCompleto, correo, numeroOrden, etapa }) => {
  ...
  `;
  await sendEmail({ to: correo, subject: `Te asignaron la orden #${numeroOrden} — Unistock`, html });
};
...
const sendAccountLockedEmail = async ({ gerenteNombre, gerenteCorreo, usuarioBloqueado }) => {
  ...
  `;
  await sendEmail({ to: gerenteCorreo, subject: `🔒 Cuenta bloqueada por intentos fallidos — ${usuarioBloqueado.nombreCompleto}`, html });
};
